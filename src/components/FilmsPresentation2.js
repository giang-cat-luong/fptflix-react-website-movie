...
export default function FilmsPresentation2() {
  const { theme, toggle, dark } = useContext(ThemeContext)
  const [APIData, setAPIData] = useState([])
  const baseURL = 'https://63609d88af66cc87dc172e0f.mockapi.io/movies?page=2&limit=6';
  useEffect(() => {
    fetch((baseURL))
      .then(response => {
        if (!response.ok) {
  ...
  return (
    <Section style={{ backgroundColor: theme.backgroundColor, color: theme.color }} >
      <Container >
        <Row>
          <h3 className='top'>
            <span className='title-new'>Page 2 of new release</span>
        ...
        <div className='pagination1'>
          <div className="pagination">
            ...
            <Link to='/page3' style={{ textDecoration: "none" }}>&raquo;</Link>
          </div>
        </div>
      </Container>
    </Section>
